  ...
  return (
    <NavContext.Provider value={nav}>
      <div className="sk-board" style={{ overflow: 'clip', height: '100vh', display: 'flex', flexDirection: 'column' }}>
        <a href="#main-content" className="sr-only" style={{ position:'absolute',width:1,height:1,padding:0,margin:-1,overflow:'hidden',clip:'rect(0,0,0,0)',whiteSpace:'nowrap',border:0 }}
          onFocus={(e) => { const el=e.currentTarget; el.style.width='auto'; el.style.height='auto'; el.style.padding='8px 16px'; el.style.margin='0'; el.style.clip='auto'; el.style.clipPath='none'; el.style.whiteSpace='normal'; el.style.position='absolute'; el.style.top='4px'; el.style.left='4px'; el.style.zIndex='50'; el.style.background='var(--paper)'; el.style.border='2px solid var(--ink)'; el.style.borderRadius='var(--sk-radius)' }}
          onBlur={(e) => { const el=e.currentTarget; el.style.width='1px'; el.style.height='1px'; el.style.padding='0'; el.style.margin='-1px'; el.style.clip='rect(0,0,0,0)'; el.style.clipPath=''; el.style.whiteSpace='nowrap'; el.style.position='absolute'; el.style.top=''; el.style.left=''; el.style.zIndex='' }}
        >跳到内容</a>
        ...
        <div style={{ flexShrink: 0, zIndex: 20, background: 'var(--paper)', borderBottom: '1px solid var(--ink-4)', paddingBottom: 8 }}>
          <div className="mx-auto max-w-7xl px-2 md:px-4 pt-4">
            ...
            <div ref={hintRef} className="flex items-center justify-between" style={{ fontSize:11, color:'var(--ink-3)', marginTop:2 }}>
              <span>{hintText}</span>
              <span style={{ color:'var(--ink-2)' }}>{cat?.label} › {SECTION_LABELS[sectionId] ?? sectionId} · {nav.activeSection+1}/{cat?.sections.length}</span>
            </div>
          </div>
        </div>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      fontSize: 12, fontFamily: 'Kalam, Caveat, system-ui, sans-serif',
                      transition: 'background 0.15s, border-color 0.15s',
                    }}
                  >
                    <div className="sk-mono" style={{ fontSize:9, color: isActive ? 'var(--accent)' : 'var(--ink-4)', marginBottom:1 }}>
                      {si + 1}
                    </div>
                    <div className="truncate">{SECTION_LABELS[sid] ?? sid}</div>
                  </button>
                )
              })}
            </nav>
              ...
              <div key={cat.id} style={{ display:nav.activeCategory===ci?'flex':'none', flexDirection:'column', height:'100%' }}
                ...
                <div className="px-2 md:px-4 py-4" style={{ width:'100%', height:'100%', overflow:'hidden', display:'flex', flexDirection:'column' }}>
                  {cat.sections.map((sectionId, si) => {
                    ...
                    return (
                      <section key={sectionId} id={`nav-${sectionId}`}
